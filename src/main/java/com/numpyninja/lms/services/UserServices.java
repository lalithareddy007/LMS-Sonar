...
public class UserServices implements UserDetailsService {
    ...
    public UserDto createUserLoginWithRole(UserLoginRoleDTO newUserLoginRoleDto) throws InvalidDataException, DuplicateResourceFoundException {
        ...
        if (newUserLoginRoleDto != null) {
            ...
            if (userList.size() > 0) {
                boolean isPhoneNumberExists = checkDuplicatePhoneNumber(userList, newUserLoginRoleDto.getUserPhoneNumber(),newUserLoginRoleDto.getUserId());
            ...
            /** checking if email is in proper email format **/
            String userEmailId = newUserLoginRoleDto.getUserLogin().getUserLoginEmail();
            if(userEmailId == null || !userEmailId.matches(Constants.REGEX_EMAIL_ID)){
                throw new InvalidDataException("User email Id is required and should be in proper email format");
            }
	...
	public UserDto updateUser(UserDto updateuserDto, String userId)
        ...
        if (userId == null) {
            ...
            if (userById.isEmpty()) {
                ...
                List<User> userList = userRepository.findAll();
                if (userList.size() > 0) {
                    boolean isPhoneNumberExists = checkDuplicatePhoneNumber(userList, updateuserDto.getUserPhoneNumber(),updateuserDto.getUserId());
                    if (isPhoneNumberExists) {
                        throw new DuplicateResourceFoundException("Failed to update User as phone number "
                                + updateuserDto.getUserPhoneNumber() + " already exists !!");
                    }
                }
                String allCountryRegex = "^(\\+\\d{1,3}( )?)?((\\(\\d{3}\\))|\\d{3})[- .]?\\d{3}[- .]?\\d{4}$";
...
//            throw new ResourceNotFoundException("UserID: " + userId + " Cannot be deleted as the User is assigned to a Batch/Program ");
     ...
     **/
    private boolean checkDuplicatePhoneNumber(List<User> userList, long phoneNumber,String userId) {
        boolean isUserPresent = false;
    // while updating the user, it checks for duplicate phone number with all users
    // other than the current user whose info is getting updated
        for (User user : userList) {
            if ((user.getUserPhoneNumber() == phoneNumber) && (!user.getUserId().equals(userId))) {
                isUserPresent = true;
                break;
            }
        }
